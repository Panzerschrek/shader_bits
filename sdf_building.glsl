...
float sdCone( vec3 p, vec2 angle_cos_sin )
{
	float u= length(p.xz);
	float v= p.y;

	return u * angle_cos_sin.y + v * angle_cos_sin.x;
}
...
float sdYInfiniteCylinder( vec3 p, float radius )
{
	return length( p.xz ) - radius;
}

float sdXInfiniteCylinder( vec3 p, float radius )
{
...
float opIntersection( float d0, float d1 )
{
	return max( d0 ,d1 );
}

float opSubtraction( float d0, float d1 )
{
